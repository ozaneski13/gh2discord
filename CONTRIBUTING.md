...
3. Run the release dry run and wait for it to go green:
   ...
   sdist and wheel with the hash-pinned tools, signs them with Sigstore,
   round-trips them through the same upload/download-artifact pins as
...
pushes and deletions), immutable releases, "Require actions to be pinned to
a full-length commit SHA", the `pypi` environment's deployment rule (tags
matching `v*` only), and the PyPI trusted publisher binding (project
`gh2discord`, workflow `publish.yml`, environment `pypi`).
